...
async def _(
    ...
    if sign_config.bg_mode == 1:
        ...
        async with httpx.AsyncClient() as client:
            ...
            with open(
                os.path.dirname(os.path.abspath(__file__)) + "/sign_bg.png", "wb"
            ) as f:
                f.write(response.content)
        # 调整大小
        sign_bg = Image.open(
            os.path.dirname(os.path.abspath(__file__)) + "/sign_bg.png"
        ).convert("RGBA")
        ...
        # 背景阴影
        shadow = Image.open(
            os.path.dirname(os.path.abspath(__file__)) + "/image/shadow.png"
        ).convert("RGBA")
    else:
        sign_bg_list = os.listdir(
            os.path.dirname(os.path.abspath(__file__)) + "/image/sign_bg"
        )
        sign_bg = Image.open(
            os.path.dirname(os.path.abspath(__file__))
            + f"/image/sign_bg/{random.choice(sign_bg_list)}"
        )
...
async def _(
    ...
    # 收集册
    row_num = (
        len_card // COL_NUM if len_card % COL_NUM != 0 else len_card // COL_NUM - 1
    )
    base = Image.open(FRAME_DIR_PATH + "/frame.png")
    base = base.resize(
        (
            40 + COL_NUM * 80 + (COL_NUM - 1) * 10,
    ...
    for i in sorted(new_dictionary.items(), key=lambda x: x[1], reverse=True):
        ...
        try:
            ...
            pass
    rank_text = f"好感排行: \n{rank_text}......\n当前排名: {rank_num}"
    rank_img = Text2Image.from_text(rank_text, 15, fill="black").to_image(
        bg_color="white"
    )
    output = BytesIO()
